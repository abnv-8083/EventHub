    ...
    if (req.session) {
        req.session.destroy((err) => {
            if (err) {
                ...
                return sendResponse(res, 500, false, "Failed to terminate the active session");
            }
            return sendResponse(res, HTTP_STATUS.ACCEPTED, true, "Organizer log out successful", { redirect: '/organizer/login' });
        });
    } else {
        return sendResponse(res, HTTP_STATUS.ACCEPTED, true, "Organizer log out successful", { redirect: '/organizer/login' });
    }
