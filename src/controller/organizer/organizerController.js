import * as Organizer from '../../repositories/organizer/organizerQueries.js'
...
export const getDashboard = async (req, res) => {
    try {
        ...
        const organizer = await Organizer.getOrganizerById(userId);
...
export const getProfile = async (req, res) => {
    try {
        ...
        const [organizer, user] = await Promise.all([
            Organizer.getOrganizerById( userId ),
            User.fetchUserById(userId, '-password')
        ]);
        ...
        return sendResponse(res,error.statusCode,false,error.message||'Server Error')
    }
}
