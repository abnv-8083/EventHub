...
export const verifyOTP = async (email, otp) => {
    ...
    if (!storedOtp) {
        throw new AppError("OTP expired or not found. Please request a new code and try again.", HTTP_STATUS.GONE);
    }

    if (storedOtp.otp !== otp) {
        throw new AppError("The code you entered is incorrect. Double-check the 6-digit OTP sent to your email (check spam folder as well).", HTTP_STATUS.BAD_REQUEST);
    }
...
export const resendOtp = async (email, name) => {
    ...
    if (data.resendLock && Date.now() < data.resendLock) {
        ...
        const error = new AppError(`Please wait ${remainingSeconds} seconds before resending.`, HTTP_STATUS.TOO_MANY_REQUESTS);
...
export const resetUserPassword = async (email, newPassword) => {
    ...
    await user.updatePasswordByEmail(email, hashPass);
    // Automatically activate account upon password reset (as they've just verified via OTP)
    await user.updateStatusByEmail(email, "Active");
